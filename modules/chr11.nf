...
process chr11_split {
    ...
    script:
    return """
    /usr/bin/samtools view -b merged_sorted.bam 11 > chr_11.bam
    """
}
...
process chr11_RG {
    ...
    output:
    tuple path("rg_added_chr11.bam"), path("rg_added_chr11.bai")
...
process chr11_varcall {
    ...
    perl /genomics-packages/BisSNP-0.82.2/sortByRefAndCor.pl \\
        ...
        --tmp \${TMP_DIR} \\
        chr11_raw.vcf \\
        /mnt/mohammadi/home/kganapathy/cloudasm_data/hg19/reference/human_g1k_v37.fasta.fai > chr11_sorted.vcf

    java -Xmx10g -Djava.io.tmpdir=\${TMP_DIR} -jar /genomics-packages/BisSNP-0.82.2/BisSNP-0.82.2.jar \\
        -L 11 \\
        -R \${REF_GENOME} \\
        -T VCFpostprocess \\
        -oldVcf chr11_sorted.vcf \\
        -newVcf chr11_filtered.vcf \\
        -snpVcf chr11_sorted.vcf \\
        -o chr11_snp_summary.txt \\
        -maxCov 200 \\
        -minSNPinWind 2
    """
}
...
process chr11_annotateRDS {
    ...
    output:
    path("chr11_derASM_annotated.csv")
