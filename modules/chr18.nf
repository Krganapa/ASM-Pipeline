...
process chr18_split {
    ...
    script:
    return """
    /usr/bin/samtools view -b merged_sorted.bam 18 > chr_18.bam
    """
}
...
process chr18_RG {
    ...
    output:
    tuple path("rg_added_chr18.bam"), path("rg_added_chr18.bai")
...
process chr18_varcall {
    ...
    perl /genomics-packages/BisSNP-0.82.2/sortByRefAndCor.pl \\
        ...
        --tmp \${TMP_DIR} \\
        chr18_raw.vcf \\
        /mnt/mohammadi/home/kganapathy/cloudasm_data/hg19/reference/human_g1k_v37.fasta.fai > chr18_sorted.vcf

    java -Xmx10g -Djava.io.tmpdir=\${TMP_DIR} -jar /genomics-packages/BisSNP-0.82.2/BisSNP-0.82.2.jar \\
        -L 18 \\
        -R \${REF_GENOME} \\
        -T VCFpostprocess \\
        -oldVcf chr18_sorted.vcf \\
        -newVcf chr18_filtered.vcf \\
        -snpVcf chr18_sorted.vcf \\
        -o chr18_snp_summary.txt \\
        -maxCov 200 \\
        -minSNPinWind 2
    """
}
...
process chr18_annotateRDS {
    ...
    output:
    path("chr18_derASM_annotated.csv")
