...
process chr15_split {
    ...
    script:
    return """
    /usr/bin/samtools view -b merged_sorted.bam 15 > chr_15.bam
    """
}
...
process chr15_RG {
    ...
    output:
    tuple path("rg_added_chr15.bam"), path("rg_added_chr15.bai")
...
process chr15_varcall {
    ...
    perl /genomics-packages/BisSNP-0.82.2/sortByRefAndCor.pl \\
        ...
        --tmp \${TMP_DIR} \\
        chr15_raw.vcf \\
        /mnt/mohammadi/home/kganapathy/cloudasm_data/hg19/reference/human_g1k_v37.fasta.fai > chr15_sorted.vcf

    java -Xmx10g -Djava.io.tmpdir=\${TMP_DIR} -jar /genomics-packages/BisSNP-0.82.2/BisSNP-0.82.2.jar \\
        -L 15 \\
        -R \${REF_GENOME} \\
        -T VCFpostprocess \\
        -oldVcf chr15_sorted.vcf \\
        -newVcf chr15_filtered.vcf \\
        -snpVcf chr15_sorted.vcf \\
        -o chr15_snp_summary.txt \\
        -maxCov 200 \\
        -minSNPinWind 2
    """
}
...
process chr15_annotateRDS {
    ...
    output:
    path("chr15_derASM_annotated.csv")
