...
process chr7_split {
    ...
    script:
    return """
    /usr/bin/samtools view -b merged_sorted.bam 7 > chr_7.bam
    """
}
...
process chr7_RG {
    ...
    output:
    tuple path("rg_added_chr7.bam"), path("rg_added_chr7.bai")
...
process chr7_varcall {
    ...
    perl /genomics-packages/BisSNP-0.82.2/sortByRefAndCor.pl \\
        ...
        --tmp \${TMP_DIR} \\
        chr7_raw.vcf \\
        /mnt/mohammadi/home/kganapathy/cloudasm_data/hg19/reference/human_g1k_v37.fasta.fai > chr7_sorted.vcf

    java -Xmx10g -Djava.io.tmpdir=\${TMP_DIR} -jar /genomics-packages/BisSNP-0.82.2/BisSNP-0.82.2.jar \\
        -L 7 \\
        -R \${REF_GENOME} \\
        -T VCFpostprocess \\
        -oldVcf chr7_sorted.vcf \\
        -newVcf chr7_filtered.vcf \\
        -snpVcf chr7_sorted.vcf \\
        -o chr7_snp_summary.txt \\
        -maxCov 200 \\
        -minSNPinWind 2
    """
}
...
process chr7_annotateRDS {
    ...
    output:
    path("chr7_derASM_annotated.csv")
