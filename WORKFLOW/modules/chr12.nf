...
process chr12_split {
    ...
    script:
    return """
    /usr/bin/samtools view -b merged_sorted.bam 12 > chr_12.bam
    """
}
...
process chr12_RG {
    ...
    output:
    tuple path("rg_added_chr12.bam"), path("rg_added_chr12.bai")
...
process chr12_varcall {
    ...
    perl /genomics-packages/BisSNP-0.82.2/sortByRefAndCor.pl \\
        ...
        --tmp \${TMP_DIR} \\
        chr12_raw.vcf \\
        /mnt/mohammadi/home/kganapathy/cloudasm_data/hg19/reference/human_g1k_v37.fasta.fai > chr12_sorted.vcf

    java -Xmx10g -Djava.io.tmpdir=\${TMP_DIR} -jar /genomics-packages/BisSNP-0.82.2/BisSNP-0.82.2.jar \\
        -L 12 \\
        -R \${REF_GENOME} \\
        -T VCFpostprocess \\
        -oldVcf chr12_sorted.vcf \\
        -newVcf chr12_filtered.vcf \\
        -snpVcf chr12_sorted.vcf \\
        -o chr12_snp_summary.txt \\
        -maxCov 200 \\
        -minSNPinWind 2
    """
}
...
process chr12_annotateRDS {
    ...
    output:
    path("chr12_derASM_annotated.csv")
