...
process chr2_split {
    ...
    script:
    return """
    /usr/bin/samtools view -b merged_sorted.bam 2 > chr_2.bam
    """
}
...
process chr2_RG {
    ...
    output:
    tuple path("rg_added_chr2.bam"), path("rg_added_chr2.bai")
...
process chr2_varcall {
    ...
    perl /genomics-packages/BisSNP-0.82.2/sortByRefAndCor.pl \\
        ...
        --tmp \${TMP_DIR} \\
        chr2_raw.vcf \\
        /mnt/mohammadi/home/kganapathy/cloudasm_data/hg19/reference/human_g1k_v37.fasta.fai > chr2_sorted.vcf

    java -Xmx10g -Djava.io.tmpdir=\${TMP_DIR} -jar /genomics-packages/BisSNP-0.82.2/BisSNP-0.82.2.jar \\
        -L 2 \\
        -R \${REF_GENOME} \\
        -T VCFpostprocess \\
        -oldVcf chr2_sorted.vcf \\
        -newVcf chr2_filtered.vcf \\
        -snpVcf chr2_sorted.vcf \\
        -o chr2_snp_summary.txt \\
        -maxCov 200 \\
        -minSNPinWind 2
    """
}
...
process chr2_annotateRDS {
    ...
    output:
    path("chr2_derASM_annotated.csv")
