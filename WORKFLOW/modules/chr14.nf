...
process chr14_split {
    ...
    script:
    return """
    /usr/bin/samtools view -b merged_sorted.bam 14 > chr_14.bam
    """
}
...
process chr14_RG {
    ...
    output:
    tuple path("rg_added_chr14.bam"), path("rg_added_chr14.bai")
...
process chr14_varcall {
    ...
    perl /genomics-packages/BisSNP-0.82.2/sortByRefAndCor.pl \\
        ...
        --tmp \${TMP_DIR} \\
        chr14_raw.vcf \\
        /mnt/mohammadi/home/kganapathy/cloudasm_data/hg19/reference/human_g1k_v37.fasta.fai > chr14_sorted.vcf

    java -Xmx10g -Djava.io.tmpdir=\${TMP_DIR} -jar /genomics-packages/BisSNP-0.82.2/BisSNP-0.82.2.jar \\
        -L 14 \\
        -R \${REF_GENOME} \\
        -T VCFpostprocess \\
        -oldVcf chr14_sorted.vcf \\
        -newVcf chr14_filtered.vcf \\
        -snpVcf chr14_sorted.vcf \\
        -o chr14_snp_summary.txt \\
        -maxCov 200 \\
        -minSNPinWind 2
    """
}
...
process chr14_annotateRDS {
    ...
    output:
    path("chr14_derASM_annotated.csv")
