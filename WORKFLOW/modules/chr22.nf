...
process chr22_split {
    ...
    script:
    return """
    /usr/bin/samtools view -b merged_sorted.bam 22 > chr_22.bam
    """
}
...
process chr22_RG {
    ...
    output:
    tuple path("rg_added_chr22.bam"), path("rg_added_chr22.bai")
...
process chr22_varcall {
    ...
    perl /genomics-packages/BisSNP-0.82.2/sortByRefAndCor.pl \\
        ...
        --tmp \${TMP_DIR} \\
        chr22_raw.vcf \\
        /mnt/mohammadi/home/kganapathy/cloudasm_data/hg19/reference/human_g1k_v37.fasta.fai > chr22_sorted.vcf

    java -Xmx10g -Djava.io.tmpdir=\${TMP_DIR} -jar /genomics-packages/BisSNP-0.82.2/BisSNP-0.82.2.jar \\
        -L 22 \\
        -R \${REF_GENOME} \\
        -T VCFpostprocess \\
        -oldVcf chr22_sorted.vcf \\
        -newVcf chr22_filtered.vcf \\
        -snpVcf chr22_sorted.vcf \\
        -o chr22_snp_summary.txt \\
        -maxCov 200 \\
        -minSNPinWind 2
    """
}
...
process chr22_annotateRDS {
    ...
    output:
    path("chr22_derASM_annotated.csv")
